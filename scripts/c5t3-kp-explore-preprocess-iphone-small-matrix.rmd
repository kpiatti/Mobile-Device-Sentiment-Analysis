...
### *Train Models*
```{r}
#train weighted knn model using iphone_df training subset
kknn_fit01 <- train(iphonesentiment ~.,
                  data = iphone_df_train,
                  method = "kknn",
                  trControl = mycontrol)
...
```{r}
#train weighted knn model using iphone_df training subset
kknn_fit02 <- train(iphonesentiment ~.,
                  data = iphone_distinct_train,
                  method = "kknn",
                  trControl = mycontrol)
...
### *Make Predictions on Test Subsets*
```{r}
kknn_fit01_preds <- kknn_fit01 %>% 
  predict(iphone_df_test)

summary(kknn_fit01_preds)
```

```{r}
kknn_fit02_preds <- kknn_fit02 %>% 
  predict(iphone_distinct_test)

summary(kknn_fit02_preds)
```
...
mean(kknn_fit01_preds == iphone_df_test$iphonesentiment)
```

```{r}
#calculate accuracy of model predictions
mean(kknn_fit02_preds == iphone_distinct_test$iphonesentiment)
```
...
### *Make Predictions on Test Subsets*
```{r}
#predict overall iphone sentiment for iphone_df_test
nb_fit01_preds <- nb_fit01 %>% 
  predict(iphone_df_test)

summary(nb_fit01_preds)
```

```{r}
#predict overall iphone sentiment for iphone_distinct_test
nb_fit02_preds <- nb_fit02 %>% 
  predict(iphone_distinct_test)

summary(nb_fit02_preds)
```
...
```{r}
#nb_fit01 prediction accuracy
mean(nb_fit02_preds == iphone_df_test$iphonesentiment)
```

```{r}
#nb_fit02 prediction accuracy
mean(nb_fit02_preds == iphone_distinct_test$iphonesentiment)
```
